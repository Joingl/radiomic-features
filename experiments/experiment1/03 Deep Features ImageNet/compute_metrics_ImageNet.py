...
def compute_ICC(df):
    ...
    results.to_csv('CSVs/ICC2 stability ImageNet.csv', index=False)
...
def compute_AUC(df):
    ...
    results.to_csv('CSVs/AUC discr power ImageNet.csv', index=False)
...
def main():
    df = pandas.read_csv('../../../feature extraction/ctp4r/03 Deep Features ImageNet/CSVs/deep features CTP4R ImageNet clean.csv')
    compute_ICC(df)
    compute_AUC(df)

    df_ICC = pandas.read_csv('CSVs/ICC2 stability ImageNet.csv')
    df_AUC = pandas.read_csv('CSVs/AUC discr power ImageNet.csv')

...
def write_header_to_csv(dict):
    with open("CSVs/gabor features CTP4R.csv", "w", newline="") as f:
        w = csv.DictWriter(f, dict.keys())
        w.writeheader()

def add_row_to_csv(dict):
    with open("CSVs/gabor features CTP4R.csv", "a", newline="") as f:
        w = csv.DictWriter(f, dict.keys())
        w.writerow(dict)
...
def extract_features():
    ...
    for k, v in largest_cs.items():
        ...
        mask = cropImageTo96x96(mask, mask)
    ...
    df = pandas.read_csv('CSVs/gabor features CTP4R.csv')

    format_CSV(df)

def format_CSV(df):
    templ = pandas.read_csv('../01 Traditional Features/CSVs/traditional features CTP4R clean.csv')
    df2 = templ.iloc[:, :4]
    ...
    cols = [col for col in df.columns if (col[0:9] == 'original_') | (col in ['group', 'scan_id', 'class', 'mask size', 'feature_index'])]
    df = df[cols]
    cols2 = cols[5:]

    for c in cols2:  #original_firstorder_10Percentile, ...
    ...
    df2.to_csv('CSVs/gabor features CTP4R clean.csv', index=False)
